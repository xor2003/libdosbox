...
`adlib_start_stream` initializes 20-byte records passed in `BX`. The labeled source now defines these as `AdlibRuntimeRecord`:
...
The data that started at old label `unk_122BC` is not a fixed-size struct. It is a group of variable-length bytecode streams interpreted by `adlib_interpret_stream` through `AdlibRuntimeRecord.arr_data_ptr`.
...
- `adlib_start_stream`: initializes one `AdlibRuntimeRecord`.
