...
public:
	explicit CircularBuffer(size_t size)
	        : _buff{std::unique_ptr<T[]>(new value_type[size])},
	          _max_size{size}
	{}
	...
	CircularBuffer &operator=(const CircularBuffer &other)
	{
		if (this != &other) {
			...
			_head = other._head;
			_tail = other._tail;
			std::copy(other.data(), other.data() + _max_size,
			          _buff.get());
		}
		return *this;
	}
	...
	{
		return sizeof(value_type) * _max_size;
	};
...
private:
	...
	public:
		...
		friend BufferIterator operator+(BufferIterator lhsiter,
		                                difference_type n)
		{
			lhsiter._index += n;
			return lhsiter;
		...
		friend BufferIterator operator+(difference_type n,
		...
		friend difference_type operator-(const BufferIterator &lhsiter,
...
template <typename T>
inline typename CircularBuffer<T>::size_type CircularBuffer<T>::capacity() const
{
	return _max_size;
}
...
template <typename T>
inline typename CircularBuffer<T>::size_type CircularBuffer<T>::size() const
{
	//	std::lock_guard<std::mutex> _lck(_mtx);
	return _size;
}
...
template <typename T>
inline typename CircularBuffer<T>::reference CircularBuffer<T>::front()
{
	//	std::lock_guard<std::mutex> _lck(_mtx);
	if (empty())
...
template <typename T>
inline typename CircularBuffer<T>::reference CircularBuffer<T>::back()
{
	//	std::lock_guard<std::mutex> _lck(_mtx);
	if (empty())
...
template <typename T>
inline typename CircularBuffer<T>::const_reference CircularBuffer<T>::front() const
{
	//	std::lock_guard<std::mutex> _lck(_mtx);
	if (empty())
		throw std::length_error("front function called on empty buffer");
...
template <typename T>
inline typename CircularBuffer<T>::const_reference CircularBuffer<T>::back() const
{
	//	std::lock_guard<std::mutex> _lck(_mtx);
	if (empty())
		throw std::length_error("back function called on empty buffer");
...
template <typename T>
inline void CircularBuffer<T>::push_back(const T &data)
{
	//	std::lock_guard<std::mutex> _lck(_mtx);
	_buff[_head] = data;
...
template <typename T>
inline void CircularBuffer<T>::push_back(T &&data) noexcept
{
	//	std::lock_guard<std::mutex> _lck(_mtx);
...
template <typename T>
inline typename CircularBuffer<T>::reference CircularBuffer<T>::operator[](size_t index)
{
	//	std::lock_guard<std::mutex> _lck(_mtx);
...
template <typename T>
inline typename CircularBuffer<T>::const_reference CircularBuffer<T>::operator[](
        size_t index) const
{
	//	std::lock_guard<std::mutex> _lck(_mtx);
	if ((index < 0) || (index >= _size))
...
template <typename T>
inline typename CircularBuffer<T>::reference CircularBuffer<T>::at(size_t index)
{
	//	std::lock_guard<std::mutex> _lck(_mtx);
	if ((index < 0) || (index >= _size))
		throw std::out_of_range("Index is out of Range of buffer size");
...
template <typename T>
inline typename CircularBuffer<T>::const_reference CircularBuffer<T>::at(size_t index) const
{
	//	std::lock_guard<std::mutex> _lck(_mtx);
	if ((index < 0) || (index >= _size))
		throw std::out_of_range("Index is out of Range of buffer size");
...
template <typename T>
inline typename CircularBuffer<T>::iterator CircularBuffer<T>::begin()
{
	//	std::lock_guard<std::mutex> _lck(_mtx);
	iterator iter;
...
template <typename T>
inline typename CircularBuffer<T>::const_iterator CircularBuffer<T>::begin() const
{
	//	std::lock_guard<std::mutex> _lck(_mtx);
	const_iterator iter;
	iter._ptrToBuffer = this;
...
template <typename T>
inline typename CircularBuffer<T>::iterator CircularBuffer<T>::end()
{
	//	std::lock_guard<std::mutex> _lck(_mtx);
	iterator iter;
	iter._ptrToBuffer = this;
...
template <typename T>
inline typename CircularBuffer<T>::const_iterator CircularBuffer<T>::end() const
{
	//	std::lock_guard<std::mutex> _lck(_mtx);
	const_iterator iter;
	iter._ptrToBuffer = this;
...
template <typename T>
inline typename CircularBuffer<T>::iterator CircularBuffer<T>::rbegin() noexcept
{
	//	std::lock_guard<std::mutex> _lck(_mtx);
	iterator iter;
	iter._ptrToBuffer = this;
...
template <typename T>
inline typename CircularBuffer<T>::const_iterator CircularBuffer<T>::rbegin() const noexcept
{
	//	std::lock_guard<std::mutex> _lck(_mtx);
	const_iterator iter;
	iter._ptrToBuffer = this;
...
template <typename T>
inline typename CircularBuffer<T>::iterator CircularBuffer<T>::rend() noexcept
{
	//	std::lock_guard<std::mutex> _lck(_mtx);
	iterator iter;
	iter._ptrToBuffer = this;
...
template <typename T>
inline typename CircularBuffer<T>::const_iterator CircularBuffer<T>::rend() const noexcept
{
	//	std::lock_guard<std::mutex> _lck(_mtx);
	const_iterator iter;
	iter._ptrToBuffer = this;
