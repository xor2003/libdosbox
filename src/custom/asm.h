...
#if DOSBOX_CUSTOM
extern void log_regs_dbx(const char *file,
                         int line,
                         const char *instr,
                         const CPU_Regs &r,
                         const Segments &s);
...
struct flagBits {
	unsigned int _CF : 1, unused1 : 1, _PF : 1, unused2 : 1, _AF : 1,
	        unused3 : 1, _ZF : 1, _SF : 1, _TF : 1, _IF : 1, _DF : 1, _OF : 1;
};
...
{
	return ((const db *const)&m < (const db *const)a) &&
	       ((const db *const)&m + 16 * 1024 * 1024 > (const db *const)a);
}
...
inline void check_type(const S &)
{
	/*
	  ...
	  if (addr >= (0x1920+0x100) && addr < (0x1920+0x10000) && (
	*(S*)(((db*)&m2c::types)+addr) )==0 && s !=0) log_debug("Read of uninit
	addr:%zx size:%zd
...
static inline db getdata(const db &s)
{
	if (m2c::isaddrbelongtom(&s)) {
		check_type(s);
		//   if (collect_rt_info)
		//   m2c::shadow_memory.collect_data((db*)&s, sizeof(s));
...
static inline dd getdata(const long &s)
{
	if (m2c::isaddrbelongtom(&s)) {
		check_type(s);
		//   if (collect_rt_info)
		//   m2c::shadow_memory.collect_data((db*)&s, 4);
		return mem_readd((db *)&s - (db *)&m);
	} else
		return s;
}
...
static inline void setdata(db *d, db s)
{
	if (m2c::isaddrbelongtom(d)) {
		set_type(*d);
		//   if (collect_rt_info)
		//   m2c::shadow_memory.collect_data((db*)d, sizeof(*d));
...
static inline void setdata(db *d, db s)
{
	...
	if (m2c::isaddrbelongtom(d) && d < ((db *)&m) + 0xc0000 &&
	    d >= ((db *)&m) + 0xa0000) {
		dw di = d - ((db *)&m) - 0xa0000;
		SDL_SetRenderDrawColor(renderer, vgaPalette[3 * s + 2],
		                       vgaPalette[3 * s + 1], vgaPalette[3 * s],
		                       255);
...
template <class D>
inline size_t shiftmodule(D dest, size_t shift) // get module of shift steps
                                                // based on destiantion size in
                                                // bits
{
...
{
	dest = (bit >= 0)
	               ? (((dest) & (~nthbitone(dest, bit))) | ((src & 1) << bit))
	               : dest;
}

inline static db LSB(dd a)
{
	return a & 1;
} // get lower bit
...
template <>
OPTINLINE void PUSH_<short int>(short int a)
{
	fix_segs();
	CPU_Push16(a);
}
...
template <>
OPTINLINE void PUSH_<int>(int a)
{
	fix_segs();
	CPU_Push32(a);
}

OPTINLINE void POP_(dw &a)
{
	fix_segs();
	a = CPU_Pop16();
}

OPTINLINE void POP_(dd &a)
{
	fix_segs();
	a = CPU_Pop32();
}
...
#endif
	memcpy(&a, m2c::raddr_(ss, stackPointer), sizeof(a));
	stackPointer += sizeof(a);
}
#endif
...
#define RM_TO_LINEAR(addr) \
	(((((size_t)addr) & 0xFFFF0000) >> 12) + (((size_t)addr) & 0xFFFF))
...
#define AFFECT_SF_(f, a) \
	{ \
		AFFECT_SF(ISNEGATIVE(f, a)); \
	}
...
MYINLINE void SHR_(D &a, const S &b, m2c::eflags &m2cflags)
{
	if (b) {
		...
		AFFECT_ZFifz(res);
		AFFECT_SF_(res, res);
		// log_debug("SHR a=%x b=%x h=%x res=%x
...
MYINLINE void ROR_(D &a, S b, m2c::eflags &m2cflags)
{
	if (b) {
		...
		a = ((a) >> (m2c::shiftmodule(a, b)) |
		     a << (m2c::bitsizeof(a) - (m2c::shiftmodule(a, b))));
		D highestbitset = (1 << (m2c::bitsizeof(a) - 1));
...
MYINLINE void RCL_(D &op1, C op2, m2c::eflags &m2cflags)
{
	...
	if (lf_var2b == 1) {
		...
		lf_resw = (lf_var1w << lf_var2b) | (cf << (lf_var2b - 1)) |
...
MYINLINE void SHRD_(D &Destination, D Source, size_t Count, m2c::eflags &m2cflags)
{
	if (Count != 0) {
		...
		if (TCount > m2c::bitsizeof(Destination)) {
			SHLD_(Destination, Source,
			      2 * m2c::bitsizeof(Destination) - TCount, m2cflags);
		} else {
			AFFECT_CF(m2c::getbit(Destination, TCount - 1));
			Destination >>= TCount;
			for (int i = m2c::bitsizeof(Destination) - TCount;
			     i <= m2c::bitsizeof(Destination) - 1; ++i)
				if (i >= 0) {
					m2c::bitset(Destination,
					            m2c::getbit(Source,
					                        i + TCount -
					                                m2c::bitsizeof(Destination)),
					            i);
				}
			if (m2c::bitsizeof(Destination) - TCount < 0) {
				AFFECT_CF(m2c::getbit(Source,
				                      TCount - m2c::bitsizeof(Destination) -
				                              1));
			}
		}
	}
...
if (TCount>m2c::bitsizeof(Destination)) {SHRD_(Destination, Source,
2*m2c::bitsizeof(Destination)-TCount, m2cflags);} else
{
...
AFFECT_CF(m2c::getbit(Destination,m2c::bitsizeof(Destination)-TCount));
                ...
                for(int i = 0; i < TCount; ++i)
                        if (i>=0)
{m2c::bitset(Destination,m2c::getbit(Source,m2c::bitsizeof(Destination) - TCount
+ i),i);}
}
 }
}
...
static void SHLD_(dw &Destination, dw Source, size_t Count, m2c::eflags &m2cflags)
{
	if (Count != 0) {
		size_t TCount = Count & (2 * m2c::bitsizeof(Destination) - 1);
		if (TCount > m2c::bitsizeof(Destination)) {
			SHRD_(Destination, Source,
			      2 * m2c::bitsizeof(Destination) - TCount, m2cflags);
		} else {
			// AFFECT_CF(((Destination<<m2c::bitsizeof(Destination)+Source)
			// >> (32 - Count)) & 1);
			AFFECT_CF(m2c::getbit(Destination,
			                      m2c::bitsizeof(Destination) - TCount));
...
MYINLINE void SAR_(D &op1, const S &op2, m2c::eflags &m2cflags)
{
	if (op2) {
		...
		if (lf_var2b > m2c::bitsizeof(op1))
			lf_var2b = m2c::bitsizeof(op1);
		D highestbitset = (1 << (m2c::bitsizeof(op1) - 1));
		D lf_resw;
...
#define IMUL1_1(a) \
	{ \
		ax = ((int8_t)al) * ((int8_t)(a)); \
		AFFECT_OF(AFFECT_CF((ax & 0xff80) != 0xff80 && (ax & 0xff80) != 0)); \
		AFFECT_ZFifz(al); \
		AFFECT_SF_(al, al); \
	}
#define IMUL1_2(a) \
	{ \
		int32_t averytemporary = (int32_t)((int16_t)ax) * ((int16_t)(a)); \
		ax = averytemporary; \
		dx = averytemporary >> 16; \
		AFFECT_OF(AFFECT_CF((averytemporary & 0xffff8000) != 0xffff8000 && \
		                    (averytemporary & 0xffff8000) != 0)); \
		AFFECT_ZFifz(ax); \
		AFFECT_SF_(ax, ax); \
	}
#define IMUL1_4(a) \
	{ \
		int64_t averytemporary = (int64_t)((int32_t)eax) * ((int32_t)(a)); \
		eax = averytemporary; \
		edx = averytemporary >> 32; \
		AFFECT_OF(AFFECT_CF((averytemporary & 0xffffffff80000000) != \
		                            0xffffffff80000000 && \
		                    (averytemporary & 0xffffffff80000000) != 0)); \
		AFFECT_ZFifz(eax); \
		AFFECT_SF_(eax, eax); \
	}
#define IMUL2_2(a, b) \
	{ \
		int32_t averytemporary = ((int16_t)(a)) * ((int16_t)(b)); \
		a = averytemporary; \
		AFFECT_OF(AFFECT_CF((averytemporary >= -32768) && \
		                                    (averytemporary <= 32767) \
		                            ? false \
		                            : true)); \
	}
#define IMUL2_4(a, b) \
	{ \
		int64_t averytemporary = ((int64_t)(a)) * ((int32_t)(b)); \
		a = averytemporary; \
		AFFECT_OF(AFFECT_CF( \
		        (averytemporary >= -((int64_t)(2147483647) + 1)) && \
		                        (averytemporary <= (int64_t)2147483647) \
		                ? false \
		                : true)); \
	}
#define IMUL3_2(a, b, c) \
	{ \
		int32_t averytemporary = ((int16_t)(b)) * ((int16_t)(c)); \
		a = averytemporary; \
		AFFECT_OF(AFFECT_CF((averytemporary >= -32768) && \
		                                    (averytemporary <= 32767) \
		                            ? false \
...
#define IMUL3_4(a, b, c) \
	{ \
		int64_t averytemporary = ((int64_t)(b)) * ((int32_t)(c)); \
		a = averytemporary; \
		AFFECT_OF(AFFECT_CF( \
		        (averytemporary >= -((int64_t)(2147483647) + 1)) && \
		                        (averytemporary <= (int64_t)2147483647) \
...
#define MUL1_1(a) \
	{ \
		ax = (dw)al * (a); \
		AFFECT_OF(AFFECT_CF(ah)); \
		AFFECT_ZFifz(al); \
	}
#define MUL1_2(a) \
	{ \
		dd averytemporary = (dd)ax * (a); \
		ax = averytemporary; \
		dx = averytemporary >> 16; \
		AFFECT_ZFifz(ax); \
		AFFECT_OF(AFFECT_CF(dx)); \
	}
#define MUL1_4(a) \
	{ \
		dq averytemporary = (dq)eax * (a); \
		eax = averytemporary; \
		edx = averytemporary >> 32; \
		AFFECT_ZFifz(eax); \
		AFFECT_OF(AFFECT_CF(edx)); \
	}
#define MUL2_2(a, b) \
	{ \
		dd averytemporary = (dd)(a) * (b); \
		a = averytemporary; \
		AFFECT_ZFifz(a); \
		AFFECT_OF(AFFECT_CF(averytemporary >> 16)); \
	}
#define MUL2_4(a, b) \
	{ \
		dq averytemporary = (dq)(a) * (b); \
		a = averytemporary; \
		AFFECT_ZFifz(a); \
		AFFECT_OF(AFFECT_CF(averytemporary >> 32)); \
	}
#define MUL3_2(a, b, c) \
	{ \
		dd averytemporary = (dd)(b) * (c); \
		a = averytemporary; \
		AFFECT_ZFifz(a); \
		AFFECT_OF(AFFECT_CF(averytemporary >> 16)); \
	}
...
/*
#define IDIV1(a) {SETFLAGBIT(OF,0);if(a) {int16_t
averytemporary=ax;al=averytemporary/((int8_t)a); ah=averytemporary%((int8_t)a);
AFFECT_OF(false);}} #define IDIV2(a) {SETFLAGBIT(OF,0);if(a) {int32_t
averytemporary=(((int32_t)(int16_t)dx)<<16)|ax;
ax=averytemporary/((int16_t)a);dx=averytemporary%((int16_t)a);
AFFECT_OF(false);}} #define IDIV4(a) {SETFLAGBIT(OF,0);if(a) {int64_t
averytemporary=(((int64_t)(int32_t)edx)<<32)|eax;eax=averytemporary/((int32_t)a);edx=averytemporary%((int32_t)a);
AFFECT_OF(false);}}
*/
#define IDIV1(op1) \
	{ \
...
static /* MYINLINE*/ void IDIV2_(dw op1, _STATE *_state)
{
	...
	if (val == 0) {
		log_debug("~~fail1\n");
		CPU_Exception(0);
		__dispatch_call((cs << 16) + ip, _state);
		return;
	}
...
#define DIV4(a) \
	{ \
		if (a) { \
			uint64_t averytemporary = ((((dq)edx) << 32) | eax); \
			eax = averytemporary / (a); \
			edx = averytemporary % (a); \
			AFFECT_OF(false); \
		} \
	}
...
template <class D, class S>
MYINLINE void MOV_(D *dest, const S &src)
{
...
#define MOVZX(dest, src) \
	{ \
		dest = src; \
	}
#define MOVSX(dest, src) \
	{ \
		if (ISNEGATIVE(src, src)) { \
			dest = ((-1 ^ ((1 << (m2c::bitsizeof(src))) - 1)) | src); \
		} else { \
			dest = src; \
		} \
	}
...
#define BT(dest, src) \
	{ \
		AFFECT_CF(dest &m2c::nthbitone(dest, src)); \
	} // TODO
#define BTS(dest, src) \
	{ \
		AFFECT_CF(dest &m2c::nthbitone(dest, src)); \
		dest |= m2c::nthbitone(dest, src); \
	}
#define BTC(dest, src) \
	{ \
		AFFECT_CF(dest &m2c::nthbitone(dest, src)); \
...
#define MOVS(dest, src, destreg, srcreg, s) \
	{ \
		dest = src; \
...
#define PUSHF \
	{ \
		PUSH((m2c::MWORDSIZE)m2cflags.getvalue()); \
	}
#define POPF \
	{ \
		m2c::MWORDSIZE averytemporary; \
		POP(averytemporary); \
		m2cflags.setvalue(averytemporary); \
		m2c::defer_irqs(); \
	}

// #define PUSHF {PUSH( (dd)
// ((GET_CF()?1:0)|(GET_PF()?4:0)|(GET_AF()?0x10:0)|(GET_ZF()?0x40:0)|(GET_SF()?0x80:0)|(GET_DF()?0x400:0)|(GET_OF()?0x800:0))
// );} #define POPF {dd averytemporary; POP(averytemporary); CF=averytemporary&1;
// PF=(averytemporary&4);AF=(averytemporary&0x10);ZF=(averytemporary&0x40);SF=(averytemporary&0x80);DF=(averytemporary&0x400);OF=(averytemporary&0x800);}
#define NOP \
	{ \
		; \
	}
...
#define LAHF \
	{ \
		ah = m2cflags.getvalue(); \
	}
#define SAHF \
	{ \
		m2cflags.setvalue(ah); \
	}

#define CALLF(label, disp) \
	{ \
		PUSH(cs); \
		CALL(label, disp); \
	}
...
#define CALL(label, disp) \
	{ \
		m2c::CALL_(label, _state, disp); \
		if (disp) { \
			__disp = disp; \
		} else { \
			__disp = m2c::k##label; \
...
static void CALL_(m2cf *label, struct _STATE *_state, _offsets _i = 0)
{
	X86_REGREF
	from_callf = true;
	MWORDSIZE averytemporary8 = eip + 2;
	...
	if (debug > 2) {
		...
		if (_state) {
			++m2c::_indent;
			m2c::_str = m2c::log_spaces(_state->_indent);
		};
	}
#endif
}
...
struct StackPop {
	explicit StackPop(size_t deep = 1) : deep(deep) {}
...
// #define RETF(i) {m2c::RETF_(i); if (ip=='xy')
// {m2c::shadow_stack.decreasedeep(); return true;} else  {return
// __dispatch_call((cs<<16)+eip,0);}}
#define RETF(i) \
	{ \
		m2c::RETF_(i, _state); \
		return true; \
...
/*
#ifndef SHADOW_STACK
#define CALL(label, disp) {if (!m2c::CALL_(label, _state, disp))
{__disp=(cs<<16)+eip;goto __dispatch_call;}} #else
*/
#define CALL(label, disp) \
	{ \
		m2c::CALL_(label, _state, disp); \
...
static bool CALL_(m2cf *label, struct _STATE *_state, _offsets _i = 0)
{
	...
	try {
		label(_i, _state);
#if M2CDEBUG > 0
		if (sp != oldsp && sp != oldsp + 2)
			log_debug("~~old SP %x != SP %x\n", oldsp, sp);
#endif
...
/*
#define IRET {m2c::MWORDSIZE averytemporary11=0; POP(averytemporary11);
eip=averytemporary11;\
        POP(cs); \
        POPF; \
        return;}
*/
...
#define RDTSC \
	{ \
		dq averytemporary = realElapsedTime(); \
		eax = averytemporary & 0xffffffff; \
		edx = (averytemporary > 32) & 0xffffffff; \
	}

#if M2CDEBUG > 0

// Legacy debug macro examples were intentionally removed for formatter stability.

//    #define R(a) { m2c::run_hw_interrupts();
//    m2c::log_regs_dbx(__FILE__,__LINE__,#a, cpu_regs, Segs); {a;}} #define
//    T(a) { m2c::run_hw_interrupts(); m2c::log_regs_dbx(__FILE__,__LINE__,#a,
//    cpu_regs, Segs); {a;}} #define X(a) { m2c::run_hw_interrupts();
//    m2c::log_regs_dbx(__FILE__,__LINE__,#a, cpu_regs, Segs); {a;}}

// #elif M2CDEBUG>=4
// Legacy debug macro examples were intentionally removed for formatter
// stability.
//                          GET_SF(), GET_OF(), GET_AF(), GET_PF(), GET_IF());}
...
#define R(a) \
	{ \
		...
		m2c::run_hw_interrupts(); \
		m2c::log_regs_dbx(__FILE__, __LINE__, #a, cpu_regs, Segs); \
		{ \
			a; \
		} \
	}
...
{
	IO_WriteB(port, value); /*printf("W %x <- %x\n",port,value);*/
}
