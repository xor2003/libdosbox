...
public:
	...
	dd getvalue() const noexcept { return _value; }
	void setvalue(dd v) noexcept { _value = v; }
#define REGDEF_flags(Z) \
	inline bool set##Z##F(bool i) noexcept \
	{ \
		return (reinterpret_cast<flagsUnion *>(&_value)->bits._##Z##F = i); \
	} \
	inline bool get##Z##F() const noexcept \
	{ \
		return reinterpret_cast<flagsUnion *>(&_value)->bits._##Z##F; \
	}
	inline void reset() { _value = 0; }

	REGDEF_flags(C) REGDEF_flags(P) REGDEF_flags(A) REGDEF_flags(Z)
	        REGDEF_flags(S) REGDEF_flags(T) REGDEF_flags(I) REGDEF_flags(D)
	                REGDEF_flags(O)
};
